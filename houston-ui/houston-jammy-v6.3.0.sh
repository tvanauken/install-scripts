...
BANNER
  echo -e "${CL}"
  echo -e "${DGN}  ── Van Auken Tech - Houston / 45Drives - Ubuntu 24.04 LTS - Jammy Installer Installer ───────────────────────────────────${CL}"
...
msg_info "Applying robustness fixes to 45Drives Firmware Check"
# The 45d-firmware-check.service crashes during automated OS updates if the DNS resolver bounces.
# Injecting Restart=on-failure ensures the service survives transient network drops.
mkdir -p /etc/systemd/system/45d-firmware-check.service.d
cat <<SYSTEMD > /etc/systemd/system/45d-firmware-check.service.d/restart.conf
[Service]
Restart=on-failure
RestartSec=60
SYSTEMD
systemctl daemon-reload >> "$LOGFILE" 2>&1
msg_ok "Firmware check service fortified"


msg_info "Applying hotfixes for Cockpit Super Simple Setup"
# The 45Drives package (1.1.7-4jammy) nests the installation incorrectly and enforces an incompatible ES module type.
if [ -d "/usr/share/cockpit/super-simple-setup/super-simple-setup" ]; then
    ln -sf /usr/share/cockpit/super-simple-setup/super-simple-setup/server.js /usr/share/cockpit/super-simple-setup/server.js
    sed -i 's/"type": "module"/"type": "commonjs"/' /usr/share/cockpit/super-simple-setup/super-simple-setup/package.json
    systemctl restart houston-broadcaster-legacy.service >> "$LOGFILE" 2>&1 || true
fi
msg_ok "Super Simple Setup loop resolved"
...
echo ""
echo -e "${GN}${BLD}  Van Auken Tech - Houston / 45Drives - Ubuntu 24.04 LTS - Jammy Installer installation is complete!${CL}"
