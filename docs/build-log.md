...
3. **Python & JSON Hardware Overrides:** The script allows `dmap` to natively detect the HBA, then uses `jq` to explicitly set `"Edit Mode": true` and `"VM": false` in `server_info.json`. It also patches the `server_identifier` Python code to prevent it from overwriting the fake physical chassis. This forces the Vue.js frontend to render a complete, uncropped physical chassis (e.g., Storinator-S45).
4. **Custom Branding:** Injected SVG vector graphics and CSS to apply the Van Auken Tech theme to the Cockpit login screen.

### Automated Validation Audit Results (2026-08-23)
The script was executed completely unattended on an isolated Ubuntu 24.04 VirtualBox instance (`houston-test-vm`) simulating a 20GB OS drive and a 10GB pass-through HBA drive.
